...
object Test extends TestSuite {
  ...
  val tests: Tests = Tests {
    ...
    'test_sumScalars - {
      assert(Exercises.sumScalars(Vector2D(0, 5), Vector2D(5, 0), Vector2D(0, 5), Vector2D(5, 0)) == 0)
    }

    'test_sumCosines - {
      assert(Exercises.sumCosines(Vector2D(0, 5), Vector2D(5, 0), Vector2D(0, 5), Vector2D(5, 0)) == 0)
    }
