...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def primeFactorInner(number: Int, divisor: Int, acc: Seq[Int]): Seq[Int] = {
            if (number <= 1) acc
            else if (number % divisor == 0) primeFactorInner(number / divisor, divisor, acc :+ divisor)
            else primeFactorInner(number, divisor + 1, acc)
        }

        primeFactorInner(number, 2, Seq.empty).distinct
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortBy {
            case (_, (radius, density)) => getWeight(radius, density)
        }.collect {
            case (name, _) => name
        }
    }

    private def getWeight(radius: Double, density: Double): Double = {
        val volume = 4 / 3 * math.Pi * math.pow(radius, 3)
        density * volume
    }
