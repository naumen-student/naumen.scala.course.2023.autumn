object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        (iFrom to iTo).foldLeft(0L) { (sum, current) =>
            if (current % 3 == 0 || current % 5 == 0) sum + current else sum
        }
    }

    def primeFactor(number: Int): Seq[Int] = {
        def factorize(n: Int, i: Int = 2, acc: Seq[Int] = Seq()): Seq[Int] = {
            if (n <= 1) acc.distinct
            else if (n % i == 0) factorize(n / i, i, acc :+ i)
            else if (i * i > n) acc :+ n
