...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    ...
    var step = 2
    val prime_array = ArrayBuffer[Int]()
    while (step <= temp_number) {
      if (temp_number % step == 0) {
        prime_array += step
        temp_number /= step
      }
      else { // Нахождение следующего простого числа
        var continue_loop = true
        while (step <= temp_number && continue_loop) {
          step += 1
          val check_prime = for {i <- 2 until step
                                 if step % i == 0
                                 } yield i
          if (check_prime.isEmpty) {
            continue_loop = false
          }
        }
      }
    }
    prime_array.distinct
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray
      .toSeq
      .sortBy{ case (name: String, (radius: Int, density: Double)) => 4/3 * java.lang.Math.pow(radius, 3) * java.lang.Math.PI * density }
      .map { case (name: String, (radius: Int, density: Double)) => name }
  }
