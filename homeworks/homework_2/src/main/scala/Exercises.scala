...
object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).filter(f => f % 3 == 0 || f % 5 == 0).sum
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, function: Function2[Vector2D, Vector2D, Double], rightVec0: Vector2D, rightVec1: Vector2D): Double =
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq
          .sortBy { case (_: String, (radius: Int, density: Double)) => math.pow(radius, 3) * density * math.Pi }
          .map { case (name: String, _: (Int, Double)) => name }
    }
