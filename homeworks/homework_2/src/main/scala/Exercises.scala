...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        val numbers = iFrom to iTo
        numbers.filter(num => num % 3 == 0 || num % 5 == 0).sum
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def go(num: Int, fact: Int, res: List[Int], lastDiv: Int): Seq[Int] = {
            num match {
                case _ if num <= 1 => res
                case _ if num % fact == 0 && lastDiv == fact => go(num / fact, fact, res, fact)
                case _ if num % fact == 0 => go(num / fact, fact, res :+ fact, fact)
                case _ => go(num, fact + 1, res, fact)
            }
        }
        go(number.abs, 2, Nil, 0)
    }
