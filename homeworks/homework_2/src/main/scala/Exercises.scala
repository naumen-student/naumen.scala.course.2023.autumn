object Exercises {
  ...
  на 3 или на 5.*/
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).filter(num => num % 3 == 0 || num % 5 == 0).sum
  ...
  def primeFactor(number: Int): Seq[Int] = {
    for (i <- 2 to number - 1) {
      if (number % i == 0) {
        return (primeFactor(i).toList ::: primeFactor(number / i).toList).toSet.toSeq;
      }
    }
    return List(number).toSeq;
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = ballsArray
    .map { case (material, (radius, density)) =>
      (material, (4.0 / 3.0) * java.lang.Math.PI * Math.pow(radius, 3) * density)
    }
    .toSeq
    .sortBy { case (_, mass) =>
      mass
