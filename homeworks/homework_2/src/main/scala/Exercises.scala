...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    ...
    for {i <- 2 to java.lang.Math.sqrt(x).toInt} {
      while (x % i == 0) {
        if (!ans.contains(i)) {
          ans += i
        }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray.toSeq.sortBy {
      case (name: String, (radius: Int, density: Double)) => density * math.pow(radius, 3)
    }.map {
      case (name: String, (radius: Int, density: Double)) => name
    }
  }
