object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long =
      (iFrom to iTo).fold(0) { case (acc, num) => if (num % 3 == 0 || num % 5 == 0) acc + num else acc}
  ...
  def primeFactor(number: Int): Seq[Int] = {
    ...
    def loop(num: Int, devider: Int, lastdevider: Int, deviders: Seq[Int]): Seq[Int] = num match {
      case 1 => deviders
      case _ if num % devider == 0 =>
        loop(
          num / devider,
          devider,
          devider,
          if (devider == lastdevider) deviders else deviders :+ devider
        )
      case _ => loop(num, devider + 1, devider, deviders)
    }
    loop(Math.abs(number), 2, 1, Seq.empty)
  }
    ...
    */
    ...
    private def ballVolume(radius: Double) = 4 / 3 * Math.PI * Math.pow(radius, 3)

    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
      ballsArray.toSeq.sortBy { case (_, (r, d)) => ballVolume(r) * d }.map(_._1)
