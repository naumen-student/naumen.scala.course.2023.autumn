...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        @tailrec
        def rec(acc: Long, current: Int): Long = {
            if (current == iTo) return acc
            else {
                if (current % 5 == 0 || current % 3 == 0) {
                    rec(acc + current, current + 1)
                }
                else {
                    rec(acc, current + 1)
                }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def rec(acc: List[Int] = Nil, cur: Int = 2, num: Int = number): List[Int] = {
            if (num == 1) acc
            else {
                if (num % cur == 0) {
                    if (acc.contains(cur)) {
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val weight = (a: String, b: Map[String, (Int, Double)]) => math.pow(b(a)._1, 3) * b(a)._2
        ballsArray.keys.toList.sortBy(a => weight(a, ballsArray))
    }
