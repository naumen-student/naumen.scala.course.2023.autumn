...
object Exercises {
  ...
  на 3 или на 5.*/
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).foldLeft(0)((sum, number) =>
  ...
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  @tailrec
  private def divideNumber(number: Int, divider: Int): Int =
  if (number % divider == 0)
    divideNumber(number / divider, divider)
  else
    number

  def primeFactor(number: Int): Seq[Int] = (2 to math.sqrt(number).toInt).foldLeft((number, Seq.empty[Int]))(
    (numberAndDividers, potentialDivider) =>
      if (numberAndDividers._1 % potentialDivider == 0)
        (divideNumber(numberAndDividers._1, potentialDivider), numberAndDividers._2.:+(potentialDivider))
      else
        numberAndDividers
  )._2 match {
    case Seq() if number == 1 => Seq()
    case Seq() => Seq(number)
    case seq: Seq[Int] => seq
  }
  ...
  */
  ...
  def sortByHeavyWeight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray.toSeq.sortBy { case (_, (radius, density)) => 4 / 3 * math.Pi * math.pow(radius, 3) * density }.map(x => x._1)
  }
}