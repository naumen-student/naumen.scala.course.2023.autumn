...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def loop(number: Int, div: Int = 2, acc: Seq[Int] = Seq.empty[Int]): Seq[Int] = {
            if (number <= 1) acc
            else if (number % div == 0 && !acc.contains(div)) loop(number / div, div, acc :+ div)
            else if (number % div == 0) loop(number / div, div, acc)
            else loop(number, div + 1, acc)
        }
        loop(number)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray
          .toSeq
          .map { case (name, (r, pl)) => (name, (4/3) * Math.PI * r*r*r * pl) }
