object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).filter(value => value % 3 == 0 || value % 5 == 0).sum
    ...
    (2 to number / 2 + 1).foldLeft(Set.empty[Int]) {
        case (acc, curr) if number % curr == 0 && !acc.exists(curr % _ == 0) => acc + curr
        case (acc, _) => acc
    }.toSeq
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
      ballsArray
        .mapValues {
        case (radius, density) => 4 / 3 * java.lang.Math.PI * radius * radius * radius * density
      }
        .toSeq
