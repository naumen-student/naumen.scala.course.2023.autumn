...
object Exercises {
  ...
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long =
    (iFrom to iTo).filter(num => num % 3 == 0 || num % 5 == 0).sum
  ...
  def primeFactor(number: Int): Seq[Int] = {
    @tailrec
    def findFactors(factors: Seq[Int], num: Int, divisor: Int): Seq[Int] = {
      if (num <= 1) factors
      else if (num % divisor != 0) findFactors(factors, num, divisor + 1)
      else if (factors.contains(divisor))
        findFactors(factors, num / divisor, divisor)
      else findFactors(factors :+ divisor, num / divisor, divisor)
    }

    findFactors(Seq(), number, 2)
  }
  ...
  def sortByHeavyweight(
    ...
    ballsArray.toSeq
      .sortBy { case (_, (r, d)) => calcWeight(r, d) }
      .map { case (name, _) => name }

  def calcWeight(radius: Int, density: Double): Double =
    4.0 / 3.0 * Pi * pow(radius, 3) * density
}
