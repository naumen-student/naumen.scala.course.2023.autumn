...
object Exercises {
    ...
    def divBy3Or7(iFrom: Int, iTo: Int): Seq[Int] = divByOneOfList(iFrom, iTo, List(3, 7))


    def divByOneOfList(iFrom: Int, iTo: Int, list: List[Int]): Seq[Int] = {
        for {i <- iFrom to iTo
             if list.exists(v => i % v == 0)
             } yield i
    }
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = divByOneOfList(iFrom, iTo, List(3, 5)).sum
    ...
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def primeFactor(number: Int): Seq[Int] = (2 to number.abs).filter(v => number %v == 0 && isPrime(v)) // O(n * sqrt(n)) = slow realization

    def isPrime(number: Int): Boolean = ! (2 to sqrt(number.abs).toInt).exists(v => number % v == 0)
    ...
    */
    ...
    def calcBallsVolume(radius: Int) : Double = 4 * java.lang.Math.PI / 3 * pow(radius, 3)

    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray
          .map(kvp => (kvp._1, kvp._2._2 * calcBallsVolume(kvp._2._1)))
          .toList
          .sortBy(kvp => kvp._2)
          .map(kvp => kvp._1)
