object Exercises {
    ...
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def primeFactor(number: Int): Seq[Int] = (2 to number).filter(i => number % i == 0 && checkPrime(i)).toSeq

    def checkPrime(number : Int): Boolean = (2 to number).filter(i => number % i == 0).length == 0
    ...
    def sumByFunc(leftVec0: Vector2D,
                  leftVec1: Vector2D,
                  delegate: (Vector2D, Vector2D) => Double ,
                  rightVec0: Vector2D,
                  rightVec1: Vector2D): Double = delegate(leftVec0, leftVec1) + delegate(rightVec0, rightVec1);
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.transform((name, values) => (4/3).toFloat * java.lang.Math.PI * java.lang.Math.pow(values._1, 3) * values._2).toSeq.sortBy(_._2).map(_._1)
}