...
object Exercises {
  ...
  на 3 или на 5.*/
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).filter(p => p % 3 == 0 || p % 5 == 0).sum
  ...
  def primeFactor(number: Int): Seq[Int] = {
    var x = number
    val result = new ArrayBuffer[Int](x + 1)
    val sqrt = math.sqrt(x).intValue()
    for (i <- 2 to sqrt) {
      while (x % i == 0) {
        result += i
        x /= i
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray.toSeq.sortBy {
      case (name: String, (radius: Int, density: Double)) =>  density * math.pow(radius, 3) * java.lang.Math.PI}
      .map{case (name: String, (r: Int, d: Double)) => name}
  }
}