object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    def _primeFactor(number: Int, divisor: Int = 2): Seq[Int] = {
      if (number <= 1) Seq.empty
      else if (number % divisor == 0) divisor +: _primeFactor(number / divisor, divisor)
      else _primeFactor(number, divisor + 1)
    }

    _primeFactor(number).distinct
  }
  ...
  def sumScalars(leftVec0: Vector2D, leftVec1: Vector2D, rightVec0: Vector2D, rightVec1: Vector2D): Double = sumByFunc(leftVec0, leftVec1, scalar, rightVec0, rightVec1)

  def sumCosines(leftVec0: Vector2D, leftVec1: Vector2D, rightVec0: Vector2D, rightVec1: Vector2D): Double = sumByFunc(leftVec0, leftVec1, cosBetween, rightVec0, rightVec1)
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray.toSeq.sortBy { case (_, (radius, density)) => 4 / 3 * math.Pi * math.pow(radius, 3) * density }.map(_._1)
  }
