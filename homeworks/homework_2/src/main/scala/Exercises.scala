...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        for (i <- 2 until sqrt(number).toInt + 1) {
            if (number % i == 0) {
                return (primeFactor(i) ++ primeFactor(number / i)).distinct
            }
        }
    ...
    */
    ...
    val balls: Map[String, (Int, Double)] =
        Map(
            ...
            "Silver" ->    (4,   4.505),  "Uranium" ->   (2,   19.04),  "Chrome" ->    (3,   7.18),
            "Cesium" ->    (7,   1.873),  "Zirconium" -> (3,   6.45)
        )

    case class Ball(name: String, density: Double, radius: Int) {
        lazy val weight = 4 / 3 * radius * radius * radius * Math.PI * density
    }
