object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        (iFrom to iTo).filter(i => i % 3 == 0 || i % 5 == 0).map(_.toLong).sum
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def factor(n: Int, a: Int = 2, factors: Seq[Int] = Seq.empty): Seq[Int] = {
            if (a * a > n) n +: factors
            else if (n % a == 0) factor(n / a, a, a +: factors)
            else factor(n, a + 1, factors)
        }

        factor(number).distinct.filter(_ > 1)
    }
