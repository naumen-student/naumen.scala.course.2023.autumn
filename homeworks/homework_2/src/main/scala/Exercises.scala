...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def factorize(n: Int, divisor: Int, factors: Seq[Int]): Seq[Int] = {
            if (n <= 1)
                factors
            else if (n % divisor == 0)
                factorize(n / divisor, divisor, factors :+ divisor)
            else
                factorize(n, divisor + 1, factors)
        }
        factorize(number, 2, Seq.empty[Int]).distinct
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.toSeq.sortBy { case (_, (radius, density)) => (4 / 3) * Math.PI * radius * radius * radius * density }.map { case (name, _) => name }
