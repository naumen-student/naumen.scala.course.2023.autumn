...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        (iFrom to iTo).filter(item => item % 3 == 0 || item % 5 == 0).sum
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def factors(n: Int, i: Int, acc: Seq[Int]): Seq[Int] = {
            if (n == 1) acc
            else if (n % i == 0) factors(n / i, i, acc :+ i)
            else factors(n, i + 1, acc)
        }

        factors(number, 2, Seq())
          .distinct
          .filter(i => (2 to Math.sqrt(i).toInt).forall(j => i % j != 0))
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ...
        ballsArray.toSeq.sortBy(ball => ball._2._1 * ball._2._1 * ball._2._1 * ball._2._2 * 4 / 3 * pi).map(_._1)
    }
}