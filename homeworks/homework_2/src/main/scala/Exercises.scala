...
object Exercises {
  ...
  def divBy3Or7(iFrom: Int, iTo: Int): Seq[Int] =
    (iFrom to iTo).filter(i => i % 3 == 0 || i % 7 == 0)
  ...
  def primeFactor(number: Int): Seq[Int] = {
    ...
    @tailrec
    def div(d: Int, i: Int): Int =
      Some(d).filter(_ % i == 0) match {
        case Some(_) => div(d / i, i)
        case _       => d
      }

    (2 to number)
      .foldLeft[(List[Int], Int)](Nil -> number) {
        case ((primes, num), i) if num % i == 0 => (i :: primes, div(num, i))
        case (acc, _)                           => acc
      }
      ._1
      .reverse
 ...
 import scala.math.{pow, Pi}
  ...
  def sortByHeavyweight(
    ...
    ballsArray.toList.map{ case b@(m, (r,d)) => (java.lang.Math.PI * pow(r,3) * d) -> b}.sorted.map(_._2._1).toSeq
}
