...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    @tailrec
    def factorization(n: Int, divider: Int, accum: Seq[Int]): Seq[Int] = {
      if (n <= 1) accum
      else if (n % divider == 0) factorization(n / divider, divider, accum :+ divider)
      else factorization(n, divider + 1, accum)
    }

    factorization(number, 2, Seq.empty).distinct
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray
      .toSeq
      .sortBy { case (_, (radius, density)) => 4 / 3 * java.lang.Math.PI * java.lang.Math.pow(radius, 3) * density }
