...
object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).toList.filter(x => x % 3 == 0 || x % 5 == 0).sum
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def findFactorsHelper(num: Int, divisor: Int, factors: Seq[Int]): Seq[Int] = {
            if (num <= 1) factors
            else if (num % divisor == 0) {
                val newFactors = if (!factors.contains(divisor)) factors :+ divisor else factors
                findFactorsHelper(num / divisor, divisor, newFactors)
            }
            else findFactorsHelper(num, divisor + 1, factors)
        }

        findFactorsHelper (number, 2, Seq () )
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = ballsArray.toSeq.sortBy { case (_, (radius, density)) => (4/3)*Math.PI*radius*radius*radius * density}.map { case (name, _) => name }
