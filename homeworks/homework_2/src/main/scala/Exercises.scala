object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        (iFrom to iTo).foldLeft(0)((summ, i) => if i % 3 == 0 || i % 5 == 0 then summ + i else summ)
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def primeFactor(number: Int): Seq[Int] = {
            (2 to number).foldLeft(Seq.empty[Int])(
                (seq, prime) =>
                    if (number % prime == 0 && !seq.exists(a => prime % a == 0))
                        seq.appended(prime)
                    else seq
            ).toSeq
        }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray
          .map((name, charactor) => charactor match {
              case (rad, pl) => name -> 4 / 3 * Math.PI * Math.pow(rad, 3) * pl
          })
          .toSeq
          .sortBy {
