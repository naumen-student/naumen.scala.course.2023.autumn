object Exercises {
    ...
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def primeFactor(number: Int, divisor: Int = 2): Seq[Int] = {
        if (number <= 1)
            Seq.empty[Int]
        else if (number % divisor == 0)
            divisor +: primeFactor(number / divisor, divisor)
        else
            primeFactor(number, divisor + 1)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def getWeight(r: Int, density: Double): Double = 4 / 3 * java.lang.Math.PI * r * r * r * density
        ballsArray.toSeq.sortBy(x => getWeight(x._2._1, x._2._2)).map(x => x._1)
    }
}