object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).filter(x => x % 3 == 0 || x % 5 == 0).sum
    ...
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def primeFactor(number: Int): Seq[Int] = (2 to number).filter(number % _ == 0).filter(
        x => !(2 until x).exists(x % _ == 0)
    )
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = ballsArray.map {
        case (name: String, (rad: Int, density: Double)) => (name, 4 / 3 * Math.pow(rad, 3) * Math.PI * density)
