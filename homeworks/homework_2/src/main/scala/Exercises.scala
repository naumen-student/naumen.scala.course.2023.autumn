...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        Range.inclusive(iFrom, iTo).filter(x => x % 3 == 0 || x % 5 == 0).sum
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def getFactors(n: Int, divider: Int = 2, result : Set[Int] = Set()): Set[Int] = {
            if (n < divider) result
            else if (n % divider == 0) getFactors(n / divider, divider, result + divider)
            else getFactors(n, divider + 1, result)
        }
        getFactors(number).toSeq
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def getWeight(ballRadius: Int, ballDensity: Double): Double = {
            (4/3)*java.lang.Math.PI*pow(ballRadius, 3)*ballDensity
        }
        ballsArray.toSeq.sortBy(x=> getWeight(x._2._1, x._2._2)).map(_._1)
    }
}