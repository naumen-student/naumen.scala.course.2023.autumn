...
object Task4 extends App {
  ...
  object EIO {
    ...
    implicit def monad[E]: MonadError[EIO, E] = new MonadError[EIO, E] {
      ...
      override def flatMap[A, B](fa: EIO[E, A])(f: A => EIO[E, B]): EIO[E, B] = fa.value.fold(EIO.error, f)

      override def raiseError[A](fa: EIO[E, A])(error: => E): EIO[E, A] = flatMap(fa)(_ => EIO.error(error))
