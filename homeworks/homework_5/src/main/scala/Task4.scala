...
object Task4 extends App {
  ...
  object EIO {
    ...
    def possibleError[A](f: => A): EIO[Throwable, A] = Try(f).fold(error, apply[A])

    implicit def monad[E]: MonadError[EIO, E] = new MonadError[EIO, E] {

      override def pure[A](value: A): EIO[E,A] = apply[A](value)

      override def flatMap[A, B](fa: EIO[E,A])(f: A => EIO[E,B]): EIO[E,B] = fa.value.flatMap(a => f(a).value) |> EIO.apply[E,B]

      override def raiseError[A](fa: EIO[E,A])(error: => E): EIO[E,A] = flatMap(fa)(_ => EIO.error[E,A](error))

      override def handleError[A](fa: EIO[E,A])(handle: E => A): EIO[E,A] =
        fa.value.left.flatMap[E,A](err => Right[E,A](handle(err))) |> EIO.apply[E,A]
    }
  }
