...
object Task2 extends App {
  ...
  case class DegreeAngle private(angle: Double) extends AnyVal
  object DegreeAngle {

    def apply(angle: Double): DegreeAngle = new DegreeAngle(Math.abs(angle) % 360)
    ...
    implicit val monoid: Monoid[DegreeAngle] = new Monoid[DegreeAngle] {
      ...
      override def combine(x: DegreeAngle, y: DegreeAngle): DegreeAngle = 
        DegreeAngle((x.angle + y.angle) % 360)
