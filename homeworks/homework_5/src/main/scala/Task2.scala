...
object Task2 extends App {
  ...
  case class DegreeAngle private (angle: Double)
  object DegreeAngle {
    private def normalizeAngle(angle: Double): Double = (360 + angle % 360) % 360
    def apply(angle: Double): DegreeAngle = new DegreeAngle(normalizeAngle(angle))
  ...
  object SquareMatrix {
    implicit def monoid[A: Monoid]: Monoid[SquareMatrix[A]] = new Monoid[SquareMatrix[A]] {
      override def empty: SquareMatrix[A] =
        SquareMatrix(Monoid[((A, A, A), (A, A, A), (A, A, A))].empty)
