  ...
  object SquareMatrix {
    implicit def monoid[A: Monoid]: Monoid[SquareMatrix[A]] = new Monoid[SquareMatrix[A]] {
      override def empty: SquareMatrix[A] = {
        val empty_elem = implicitly[Monoid[A]].empty
        SquareMatrix(
          (empty_elem, empty_elem, empty_elem),
          (empty_elem, empty_elem, empty_elem),
          (empty_elem, empty_elem, empty_elem)
        )
      }

      override def combine(x: SquareMatrix[A], y: SquareMatrix[A]): SquareMatrix[A] = SquareMatrix(x.values combine y.values)
    }
  }
