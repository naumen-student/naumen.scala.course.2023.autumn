...
object Task3 extends App {
  ...
  object WordsCount {
    implicit val monoid: Monoid[WordsCount] = new Monoid[WordsCount] {
      ...
      def combine(x: WordsCount, y: WordsCount): WordsCount =
        WordsCount((x.count ++ y.count).groupBy(_.word).map { case (word, counts) =>
  ...
  def countWords(lines: Vector[String]): WordsCount = {
    Await.result(mapReduce(lines) { line =>
      WordsCount(line.split("\\s+").groupBy(identity).map { case (word, instances) =>
        Count(word, instances.length)
      }.toSeq)
    }, 5.seconds)
  }
}
