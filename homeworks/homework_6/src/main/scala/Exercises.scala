...
object Exercises {
  ...
  def task1(r: Int, g: Int, b: Int): URIO[ColorService, Option[Color]] = {
    for {
      result <- ZIO.serviceWithZIO[ColorService](_.getColor(r, g, b)).map(elem => Some(elem)).orElse(ZIO.none)
    } yield result
  }
  ...
  def task2(size: (Int, Int)): ZIO[PictureGenerationService, GenerationError, String] =
    ZIO.serviceWithZIO[PictureGenerationService](_.generatePicture(size)).map(
      tableColours => tableColours.lines.map(
        lineColours => lineColours.map(elem => Integer.toUnsignedString(elem.getRGB)).mkString(" ")
      ).mkString("\n")
    )
  ...
  def task3(size: (Int, Int)): ZIO[PictureGenerationService with ColorService, GenerationError, Picture] =
    for {
      ...
      filledPicture <- pictureServ.fillPicture(picture, color).orElseFail(new GenerationError(
        "Возникли проблемы при заливке изображения"))
    } yield filledPicture
