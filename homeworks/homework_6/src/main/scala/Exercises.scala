...
object Exercises {
    ...
    def task2(size: (Int, Int)): ZIO[PictureGenerationService, GenerationError, String] =
        ZIO.serviceWithZIO[PictureGenerationService](
          _.generatePicture(size)
            .map(_.lines.map(_.map(_.getRGB).mkString(" ")).mkString("\n")))
    ...
    def task3(size: (Int, Int)): ZIO[PictureGenerationService with ColorService, GenerationError, Picture] =
        for {
            ...
            color <- colorServ.generateRandomColor().orElseFail(GenerateRandomColorError)
            picture <- pictureServ.generatePicture(size).orElseFail(GeneratePictureError)
            filledPicture <- pictureServ.fillPicture(picture, color).orElseFail(FillPictureError)
        } yield filledPicture
