...
object Exercises {
    ...
    def task2(size: (Int, Int)): ZIO[PictureGenerationService, GenerationError, String] =
        ZIO.serviceWithZIO[PictureGenerationService](_.generatePicture(size)).map { picture =>
            picture.lines.map { line =>
                line.map(c => Integer.toUnsignedString(c.getRGB))
                  .mkString(" ")
            }.mkString("\n")
        }
    ...
    def task3(size: (Int, Int)): ZIO[PictureGenerationService with ColorService, GenerationError, Picture] =
        for {
            colorService <- ZIO.service[ColorService.Service]
            pictureService <- ZIO.service[PictureGenerationService.Service]
            color <- colorService.generateRandomColor().mapError(_ => new GenerationError("Не удалось создать цвет"))
            picture <- pictureService.generatePicture(size).mapError(_ => new GenerationError("Ошибка генерации изображения"))
            filledPicture <- pictureService.fillPicture(picture, color).mapError(_ => new GenerationError("Возникли проблемы при заливке изображения"))
        } yield filledPicture
