...
object Exercises {
  ...
  def task2(size: (Int, Int)): ZIO[PictureGenerationService, GenerationError, String] =
    ZIO.serviceWithZIO[PictureGenerationService](_.generatePicture(size).map(picture => picture.lines.foldLeft("")((matrix, colors) => {
      matrix + colors.foldLeft("")((row, color) => if (row.isEmpty)
        row + color.getRGB.abs
      else
        row + " " + color.getRGB.abs) + "\n"
    })))
   ...
   */
  def task4(size: (Int, Int)): IO[GenerationError, Picture] = task3(size).provideLayer(ColorService.live >+> PictureGenerationService.live)
