...
object Breakfast extends ZIOAppDefault {
  ...
  def makeBreakfast(eggsFiringTime: Duration,
    ...
    for {
      eggsTask <- ZIO.sleep(eggsFiringTime).map(_ => LocalDateTime.now()).fork
      waterTask <- ZIO.sleep(waterBoilingTime).map(_ => LocalDateTime.now()).fork
      saladTask <- ZIO.sleep(saladInfoTime.tomatoTime.plus(saladInfoTime.cucumberTime))
        .map(_ => LocalDateTime.now()).fork
      waterBoiledTime <- waterTask.join
      teaTime <- ZIO.sleep(teaBrewingTime).map(_ => LocalDateTime.now())
