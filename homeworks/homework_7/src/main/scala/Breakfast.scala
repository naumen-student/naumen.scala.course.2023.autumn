...
object Breakfast extends ZIOAppDefault {
  ...
  def makeBreakfast(eggsFiringTime: Duration,
    ...
    waterBoilingTask <- ZIO.sleep(waterBoilingTime).as(LocalDateTime.now()).fork
    eggsFiringTask <- ZIO.sleep(eggsFiringTime).as(LocalDateTime.now()).fork
    saladTask <- ZIO.sleep(saladInfoTime.tomatoTime.plus(saladInfoTime.cucumberTime)).as(LocalDateTime.now()).fork

    waterTime <- waterBoilingTask.join
    teaTime <- ZIO.sleep(teaBrewingTime).as(LocalDateTime.now())
    eggsTime <- eggsFiringTask.join
