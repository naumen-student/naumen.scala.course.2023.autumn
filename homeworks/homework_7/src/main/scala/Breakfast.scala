...
object Breakfast extends ZIOAppDefault {
  ...
  def makeBreakfast(eggsFiringTime: Duration,
    ...
    for {
      eggsFiber <- ZIO.sleep(eggsFiringTime).map(_ => LocalDateTime.now).fork
      waterFiber <- ZIO.sleep(waterBoilingTime).map(_ => LocalDateTime.now).fork
      saladFiber <- ZIO.sleep(saladInfoTime.tomatoTime).flatMap(_ =>
        ZIO.sleep(saladInfoTime.cucumberTime).map(_ => LocalDateTime.now)
      ).fork
      teaFiber <- ZIO.sleep(teaBrewingTime).map(_ => LocalDateTime.now).fork
