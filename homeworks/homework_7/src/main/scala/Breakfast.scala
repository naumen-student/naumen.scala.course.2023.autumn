...
object Breakfast extends ZIOAppDefault {
  ...
  def makeBreakfast(eggsFiringTime: Duration,
    ...
    eggsTimeF <- ZIO.sleep(eggsFiringTime).as(LocalDateTime.now()).fork
    waterTimeF <- ZIO.sleep(waterBoilingTime).as(LocalDateTime.now()).fork
    saladTimeF <- ZIO.sleep(saladInfoTime.tomatoTime.plus(saladInfoTime.cucumberTime)).as(LocalDateTime.now()).fork
    waterTime <- waterTimeF.join
    teaTime <- ZIO.sleep(teaBrewingTime).as(LocalDateTime.now())
    saladTime <- saladTimeF.join
    eggsTime <- eggsTimeF.join
  } yield Map(
    "eggs" -> eggsTime,
    "water" -> waterTime,
