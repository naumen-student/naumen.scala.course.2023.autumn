...
import zio.{Clock, Duration, Fiber, Scope, ZIO, ZIOAppArgs, ZIOAppDefault}
...
object Breakfast extends ZIOAppDefault {
  ...
  def makeBreakfast(eggsFiringTime: Duration,
    ...
    val totalTimeSalad = saladInfoTime.cucumberTime plus saladInfoTime.tomatoTime

    for {
      startTime <- ZIO.succeed(LocalDateTime.now())
      eggsFryFiber <- ZIO.sleep(eggsFiringTime).as(startTime plus eggsFiringTime).fork
      waterBoilFiber <- ZIO.sleep(waterBoilingTime).as(startTime plus waterBoilingTime).fork
      saladWithSourCreamTime <- ZIO.sleep(totalTimeSalad).as(startTime plus totalTimeSalad)
      waterBoiledTime <- waterBoilFiber.join
      teaTime <- ZIO.sleep(teaBrewingTime).as(waterBoiledTime plus teaBrewingTime)
