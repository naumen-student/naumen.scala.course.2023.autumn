...
object ResuourceTraining extends ZIOAppDefault {
  ...
  def readData(filePath: String): IO[Throwable, String] =
    ZIO.acquireReleaseWith(ZIO.succeed(
      new BufferedReader(new FileReader(filePath))))(reader =>
      ZIO.succeed(reader.close()).ignore) {
  ...
  def writeData(filePath: String, data: String): ZIO[Any, Nothing, Unit] =
    ZIO.acquireReleaseWith(ZIO.succeed(
      ...
      ZIO.succeed(writer.close()).ignore) {
      writer => ZIO.succeed(writer.write(data)).map(_ => writer.flush()).ignore
    }
