...
object Exercises {
    ...
    def tailRecRecursion(items: List[Int], index: Int = 1): Int = {
        ...
        @tailrec
        def realTailRecursion(items: List[Int] = items.reverse, index: Int = items.length - index + 1, result: Int = 1): Int =
            items match {
                case head :: tail =>
                    realTailRecursion(tail, index - 1, index + (if (head % 2 == 0) 1 else -1) * head * result)
                case _ => result
            }
