...
object Exercises {
    ...
    def findSumFunctional(items: List[Int], sumValue: Int): (Int, Int) = {
        items.indices.combinations(2)
          .find { case Seq(i, j) => items(i) + items(j) == sumValue && i != j }
          .map { case Seq(i, j) => if (i < j) (j, i) else (i, j) }
          .getOrElse((-1, -1))
    }
            ...
            items match {
                case head :: tail =>
                    val newAcc = if (head % 2 == 0) {
                        acc * head + (length - index + 1)
                    } else {
                        acc * (-1 * head) + (length - index + 1)
                    }
    ...
    def functionalBinarySearch(items: List[Int], value: Int): Option[Int] = {
        ...
        def binarySearch(low: Int, high: Int): Option[Int] = {
            if (low <= high) {
                ...
                items(mid) match {
                    case iv if (iv == value) => Some(mid)
                    case iv if (iv < value) => binarySearch(mid + 1, high)
    ...
    def generateNames(namesCount: Int): List[String] = {
        ...
        List.fill(namesCount)(generateRandomName)
    }
}
